...
class Address(models.Model):
    ...
    town_area_kana = models.CharField('町域名カナ', max_length=255, null=True, blank=True)
    prefecture = models.CharField('都道府県名カナ', max_length=255)
    city = models.CharField('市区町村名', max_length=255)
    town_area = models.CharField('町域名', max_length=255, null=True, blank=True)
    is_one_town_by_multi_postal_code = models.BooleanField('一町域が二以上の郵便番号で表される場合の表示',
                                                           null=True, blank=True)
    is_need_small_area_address = models.BooleanField('小字毎に番地が起番されている町域の表示',
                                                     null=True, blank=True)
    is_chome = models.BooleanField('丁目を有する町域の場合の表示', null=True, blank=True)
    is_multi_town_by_one_postal_code = models.BooleanField('一つの郵便番号で二以上の町域を表す場合の表示',
                                                           null=True, blank=True)
    ...
    def __str__(self):
        return f'{self.postal_code}/{self.prefecture}/{self.city}/{self.town_area}'
